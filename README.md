...
在继续修改交互之前，先阅读 [数据流基线规格](docs/数据流基线规格.md)、[芯片研发性能与数据通路规格](docs/芯片研发性能与数据通路规格.md) 和 [LPU 核心数据流规格](docs/LPU核心数据流规格.md)。其中 LPU 文档是下一版交互的重点：上游 Host/GPU 简化，LPX/LPU 内部逐笔展开。
...
默认“完整请求”只接收一次 Prompt，先运行 Prefill 的代表层与循环节点并回放首个 Token，再进入 Decode；Decode 的代表层展开 GPU → Groq LPX → GPU 的完整数据路径，后续同构层用 LOOP 显示层范围、迭代次数和状态提交。结构变化的层（27B 的 Gated Attention、Flash-Next 的 PLE / QSA）保留单独代表层。传输按描述符、分段接收、背压保持与完整负载提交展开；结束后停止，需要明确重置才会重新输入。“下一 Token”保留上下文，直接进入 Decode，不重复原句的接收、分词与 Prefill。单独选择 Prefill 时，查看的是全部在 GPU 执行的前缀处理。

计算窗口悬浮在完整三维视口中，上半部分常驻 Prefill / Decode 数据 flow，下半部分展开当前算子。进入 LPU 芯片后，数据对象会直接绑定到 MEM、SXM、MXM、VXM、C2C、ICU 实体，当前事件高亮芯片内路径；总 flow 只做导航，不代替芯片内数据流。中央硬件旋转时窗口保持朝向和位置；窗口上的加减按钮、百分比复位和滚轮可单独缩放，左右菜单与底部操作栏保持原布局。
...
| 位置 | 内容与操作 |
| --- | --- |
| 左侧悬浮菜单 | 模型、Prefill / Decode、代表层、Loop 节点、同时显示的指令与数据 |
