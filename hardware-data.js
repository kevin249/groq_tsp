/* UTF-8 · 硬件执行的数值模型，所有节拍均为演示步，不是芯片时钟周期。 */
(function(root){
 'use strict';
 ...
 const chapters=[
  ...
  {title:'后续层与其他芯片',sub:'权重分布与激活传输',graph:'cluster',zones:['NORTH','SOUTH','MEMW','MEME'],color:'blue',
   question:'完整模型大于一颗芯片时，数据去哪里？',fact:'四个分片仅为示意数量；一层和一颗芯片没有固定的一一对应关系，图中卡槽布局也不是实际网络拓扑。',
   frames:[
  ...
  {title:'Token 返回，再进入芯片',sub:'选择、发送、复用历史',graph:'output',zones:['VX','MEME','PCIE'],color:'green',
   question:'用户看到的每个新片段，是怎样来的？',fact:'Token 与回答为预设演示片段。采样器和返回缓冲是服务逻辑示意，未断言它们一定在 LPU 或 CPU 上。',
   frames:[
    ...
    frame('完整示例回答已经到达用户','结束本次响应','本演示到达 5 个输出片段的长度上限，下一步结束生成。'),
 ...
 const info={
  ...
  NORTH:['片间互连','与其他 LPU 交换模型执行需要的数据。','多芯片图仅显示示意分片，并非真实拓扑。',8],
