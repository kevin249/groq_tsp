...
export class FloatingExecution {
 constructor({scene,host,panel,onChange}){
  ...
  // CSS3D 面板上的 bubble 事件可稳定到达 panel，本地处理教学窗口控制，不依赖外层 host 事件代理。
  panel.el.addEventListener('click',event=>this.handlePanelClick(event));
 ...
 handlePanelClick(event){
  const target=event.target;if(!target?.closest||target.closest('[data-teach-mode]'))return;
