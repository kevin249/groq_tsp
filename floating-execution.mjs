...
export class FloatingExecution {
 constructor({scene,host,panel,onChange}){
  ...
  this.viewport={w:1,h:1};this.drag=null;this.docked=false;
 ...
 dockBaseY(){const {h}=this.viewport,height=this.logicalHeight||454;return this.docked?Math.max(height*this.zoom/2+18,h-height*this.zoom/2-18):h/2+28;}
 setDocked(value){this.docked=!!value;this.clampOffset();this.applyPlacement();}
 clampOffset(){const {w,h}=this.viewport,width=this.logicalWidth||800,height=this.logicalHeight||454,shownW=width*this.zoom,shownH=height*this.zoom,keepX=40,keepY=30,baseY=this.dockBaseY();const cx=w/2+this.offsetX,cy=baseY+this.offsetY;const minX=keepX-shownW/2,maxX=w-keepX+shownW/2,minY=keepY-shownH/2,maxY=h-keepY+shownH/2;this.offsetX=T.MathUtils.clamp(cx,minX,maxX)-w/2;this.offsetY=T.MathUtils.clamp(cy,minY,maxY)-baseY;}
 applyPlacement(){if(!this.baseScale||!this.reference)return;const camera=this.reference,{w,h}=this.viewport,width=this.logicalWidth,height=this.logicalHeight,depth=28,unit=this.baseScale;const centerY=this.dockBaseY()+this.offsetY,local=new T.Vector3(this.offsetX*unit,(h/2-centerY)*unit,-depth),o=this.panel.obj;o.position.copy(local.applyQuaternion(camera.quaternion).add(camera.position));o.quaternion.copy(camera.quaternion);o.rotateY(-.10);o.rotateX(.045);o.rotateZ(-.015);o.scale.setScalar(unit*this.zoom);this.frame.position.copy(o.position);this.frame.quaternion.copy(o.quaternion);this.frame.scale.setScalar(this.zoom);o.updateMatrixWorld(true);this.frame.updateMatrixWorld(true);}
 layout(camera,viewport){const {w,h,x,y}=viewport;this.viewport={w,h,x,y};this.css.setSize(w,h);Object.assign(this.css.domElement.style,{left:x+'px',top:y+'px',width:w+'px',height:h+'px'});this.reference.copy(camera);this.reference.aspect=w/h;this.reference.updateProjectionMatrix();this.reference.updateMatrixWorld();const width=this.docked?Math.max(1000,Math.min(1500,w*.98)):Math.max(700,Math.min(980,w*.88)),height=this.docked?Math.max(460,Math.min(620,h*.62)):454,depth=28;this.logicalWidth=width;this.logicalHeight=height;const unit=2*depth*Math.tan(T.MathUtils.degToRad(camera.fov)/2)/h;this.baseScale=unit;this.panel.el.style.width=width+'px';this.panel.el.style.height=height+'px';this.panel.el.dataset.placement='world';const ww=width*unit,hh=height*unit,t=.045;for(let i=0;i<4;i++){const e=this.edges[i];e.position.set(i<2?0:(i===2?-ww:ww)/2,i<2?(i===0?hh:-hh)/2:0,-t);e.scale.set(i<2?ww+t:t,i<2?t:hh+t,t*2);}this.back.position.z=-t*2;this.back.scale.set(ww,hh,1);this.panel.worldSize={width:ww,height:hh};this.clampOffset();this.applyPlacement();this.updateZoomLabel();this.bindControls();}
 ...
 render(renderer){if(!this.panel.obj.visible)return;this.frame.visible=true;const clear=renderer.autoClear;renderer.autoClear=false;renderer.clearDepth();renderer.render(this.frame,this.reference);renderer.autoClear=clear;this.frame.visible=false;this.css.render(this.root,this.reference);}
}
