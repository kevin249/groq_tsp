...
(function(root){
 ...
 function create({container,assets,map,onInspect}){
  ...
  const colors=['cycle-instruction','cycle-data-valid','cycle-working','cycle-write','cycle-selected','cycle-invalid','cycle-pulse','cycle-row-selected'];
  ...
  function render(m,s,tracked){
   for(const [id,n]of Object.entries(elements)){colors.forEach(k=>n.classList.remove(k));label(id,map.nodes[id].label,['sl-label','sl-cell'].includes(map.nodes[id].kind)?13:map.nodes[id].kind==='caption'?14:undefined);n.style.display='';}
   label('WINDOW','SL '+s.superlane+' 内部 · 西侧切片与中央 VXM 的展开窗口',17);
   label('OV_TITLE','SL '+s.superlane+' 横跨各切片 · 点击行号选中；点击格子查看该 SL 内部',17);
   // 每格反映该切片在对应 SL 的本拍状态；不把单 SL 的 ALU 行当成其他 SL。
   for(let sl=0;sl<20;sl++)for(const [zone] of map.zones){
    const id='GRID_'+zone+'_'+sl,ops=m.physicalProgram.filter(i=>map.unitZone[i.unit.replace(/\d+$/,'')]===zone);
    const arrived=ops.filter(i=>i.issue+sl===s.cycle),running=ops.filter(i=>i.consume+sl<=s.cycle&&s.cycle<i.ready+sl),done=ops.filter(i=>i.ready+sl===s.cycle);
    const waiting=ops.filter(i=>i.issue+sl<=s.cycle&&s.cycle<i.consume+sl);
    let status='';
    if(done.some(i=>i.kind==='write')){status='写回';mark(id,'cycle-write');}
    else if(running.length){status=running.some(i=>i.kind==='read')?'读 SRAM':running.some(i=>i.kind==='write')?'写 SRAM':'运算';mark(id,'cycle-working');}
    else if(done.length){status=done.some(i=>i.kind==='read')?'读出':'结果';mark(id,'cycle-data-valid');}
    else if(waiting.length)status='等待';
    const endpoints=m.routes.filter(r=>(map.unitZone[r.from]===zone&&r.start+sl===s.cycle)||(map.unitZone[r.to]===zone&&r.end+sl===s.cycle));
    if(endpoints.length&&!status){status='SR 有效';mark(id,'cycle-data-valid');}
    if(sl===s.superlane)mark(id,'cycle-row-selected');
    if(arrived.length){mark(id,'cycle-instruction');status='I'+(status?' · '+status:' 到达');}
    label(id,status,zone==='SW'||zone==='SE'?11:12);
    elements[id].setAttribute('aria-label','SL '+sl+' / '+map.zones.find(z=>z[0]===zone)[1]+' / C '+s.cycle+'；指令：'+(arrived.map(i=>i.unit+' '+i.op).join('；')||'本拍无新指令')+'；状态：'+(status||'本示例未调度'));
   }
   ...
   for(const bus of ['X','W','Y']){const streams=s.streams.filter(r=>r.bus===bus),path=streams.at(-1);label('BUS_'+bus,[bus==='X'?'sX · 数据':bus==='W'?'sW · 第二路':'sY · 结果',path?path.from.replace('MEM_','M')+' → '+path.to.replace('MEM_','M'):'本拍无数据'],13);
    for(let hop=0;hop<=6;hop++){const id=bus+hop,n=elements[id];if(hop>m.p.hops){n.style.display='none';continue;}const width=784/(m.p.hops+1),x=238+map.detailOffset+hop*width;n.setAttribute('transform',`translate(${x+(width-7)/2},${map.nodes[id].y+17.5})`);const r=n.querySelector('rect');r.setAttribute('x',-(width-7)/2);r.setAttribute('width',width-7);const packet=streams.find(v=>v.hop===hop);if(packet){label(id,['SG'+packet.planes+' / SR '+hop+' · 1',short(compact(packet.values),width>150?24:13)],width>150?14:12);mark(id,bus==='Y'?'cycle-write':'cycle-data-valid',true);n.setAttribute('aria-label',`流 ${bus} 寄存器 ${hop}，周期 ${s.cycle}，SL ${s.superlane}，数据 ${compact(packet.values)}`);}else{label(id,['SR '+hop+' · valid 0','空'],13);mark(id,'cycle-invalid');}}
   }
   for(const u of s.units){const dispatch=s.physicalIssued.filter(i=>i.unit.startsWith(u.unit)),memory=u.unit.startsWith('MEM');label('ICU_'+u.unit,[memory?u.unit.replace('_',' ')+'0—3 · 四个 ICU':u.unit+' · PC#'+u.pc,dispatch.length?short(dispatch.map(i=>(memory?i.plane+':':'')+i.op.split(' ')[0]).join(' '),23):'NOP · 无新指令'],13);if(dispatch.length){mark('ICU_'+u.unit,'cycle-instruction',true);mark('OV_ICU_'+map.unitZone[u.unit],'cycle-instruction');}const owner=u.unit.startsWith('MEM')?'MW':u.unit==='MXM'?'MW0':u.unit==='SXM'?'SW':'VX';if(u.running.length)mark('TOP_'+owner,'cycle-working');if(u.instruction)mark(u.unit.startsWith('MEM')?u.unit+'_ADDR':u.unit+'_TITLE','cycle-instruction');}
   const trace=(tracked?m.physicalProgram.find(i=>i.parent===tracked):null)||s.physicalIssued[0]||m.physicalProgram.filter(i=>i.issue<=s.cycle).at(-1);if(trace){const tile=s.cycle-trace.issue;if(tile>=0&&tile<20){mark('SL'+tile,'cycle-instruction',true);const z=map.unitZone[trace.unit.replace(/\d+$/,'')];if(z)mark('GRID_'+z+'_'+tile,'cycle-instruction',true);}label('NORTH',['向北传播',tile<20?trace.unit.replace('MEM_','M'):'指令已到顶'],11);}
   mark('SL'+s.superlane,'cycle-selected');
   return svg;
  }
  function focus(key='all'){const b=map.focus[key]||map.focus.all;svg.setAttribute('viewBox',[b.x,b.y,b.w,b.h].join(' '));for(const [id,n]of Object.entries(elements)){const spec=map.nodes[id],visible=spec.x<b.x+b.w&&spec.x+spec.w>b.x&&spec.y<b.y+b.h&&spec.y+spec.h>b.y;n.setAttribute('tabindex',visible&&spec.kind!=='frame'?'0':'-1');n.setAttribute('aria-hidden',String(!visible));}}
