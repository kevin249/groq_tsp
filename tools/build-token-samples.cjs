...
(async()=>{for(const [key,name]of [['q27','Qwen3.8-27B'],['flash','Qwen3.8-Flash-Next']]){
 const base='https://huggingface.co/Qwen/'+name+'/resolve/main/',read=async file=>{const r=await fetch(base+file);if(!r.ok)throw Error(name+' '+file+' '+r.status);return r.text();};
